...
class DataHelperUnitTests : UnitTestBase() {
    ...
    fun validateIntegerInput_emptyString() {
        val input = ""
        Assert.assertEquals(false, DataHelper.isValidIntegerInput(input, inputName, null))
    }
    ...
    fun validateIntegerInput_correctString() {
        val input = "150"
        Assert.assertEquals(true, DataHelper.isValidIntegerInput(input, inputName, null))
    }
}